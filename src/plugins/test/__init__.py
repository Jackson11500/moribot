...
moritest = on_command("变猫娘",permission=SUPERUSER,priority=99)
...
async def handle_first_receive(bot: Bot, event: Event, state: T_State):
    ...
    if sb == ['null']:
        ...
        await bot.send(event,message=str(event.is_tome())+MessageSegment.at(event.user_id)+"使用了猫娘变化大法，把"+MessageSegment.at(sb[0])+"变成了["+getcatgirl()+"]")
